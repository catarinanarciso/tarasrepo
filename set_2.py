...
def caesar_cipher(message, shift):
    ...
    for letter in message:
        if letter == ' ':
            ...
            index = alphabet.index(letter)
            shifted_number = (index + shift) % len(alphabet)
            result += alphabet[shifted_number]
        else:
            result += letter
...
def shift_by_letter(letter, letter_shift):
    ...
    if letter not in alphabet or letter_shift not in alphabet:
        return letter  
    
    index_letter_shift = alphabet.index(letter_shift) 
    index_letter = alphabet.index(letter)  
    shifted_number = (index_letter + index_letter_shift) % len(alphabet) 

    return alphabet[shifted_number]
...
def scytale_cipher(message, shift):
    ...
    # Replace `pass` with your code.
    # Stay within the function. Only use the parameters as input. The function should return your answer.
    if shift <= 0 or shift > len(message):
        return "Invalid. Please provide a positive integer that does not exceed the length of message."
